...
const Sidebar = ({isMenuOpen, setIsMenuOpen}) => {
    ...
    const { chats,setSelectedChat, user, theme, setTheme, navigate, createNewChat, setChats, fetchUsersChat, setToken, axios, token } = useAppContext()
  ...
  return (
      <div className={`flex flex-col h-screen min-w-72 dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 p-5 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      ...
      {/* logo  */}
          ...
          <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-3'>
              {
                  chats.filter((chat) => chat.messages[0] ? chat.messages[0]?.content.toLowerCase().includes(search.toLowerCase()) : chat.name.toLowerCase().includes(search.toLowerCase())).map((chat) => (
                      <div onClick={() => { navigate("/"); setSelectedChat(chat); setIsMenuOpen(false) }} key={chat._id} className='p-2 px-4 dark:bg-[#57327C]/10 border border-gray-300 dark:border-[#80609F]/15 rounded-md cursor-pointer flex justify-between group'>
          ...
          <div onClick={() => { navigate('/credits');  setIsMenuOpen(false)}} className='flex items-center gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all'>
              ...
              <div className='flex flex-col text-sm'>
                  ...
                  <p className='text-xs text-gray-400'>purchase credit to use chatGPT</p>
              </div>
          </div>
          ...
          <div className='flex items-center justify-between gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md '>
              ...
              <label onChange={() => setTheme(theme === 'dark' ? 'light' : 'dark')} className='relative inline-flex cursor-pointer'>
